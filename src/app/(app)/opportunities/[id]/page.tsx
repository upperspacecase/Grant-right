...
import { currency, formatDate, relativeDeadline, typeLabel } from "@/lib/format";
    ...
    <>
        ...
        actions={
          <>
            ...
            {app ? (
              ...
              <Button asChild>
                <Link href={`/applications/new?opportunity=${opp.id}`}>
      ...
      <div className="px-4 sm:px-6 md:px-8 py-6 sm:py-8 grid grid-cols-12 gap-6 md:gap-8">
        <div className="col-span-12 lg:col-span-8 space-y-6 md:space-y-8 min-w-0">
          <section>
            <h3 className="font-heading text-xl mb-3">Eligibility</h3>
            ...
            <Card className="mt-4 p-4 text-sm">
              ...
              <ul className="text-xs space-y-1.5">
                <li className="flex items-center gap-2"><span className="size-2 rounded-full bg-accent" /> All eligibility flags satisfied by your profile.</li>
                <li className="flex items-center gap-2"><span className="size-2 rounded-full bg-accent" /> No conflicting prior-award wait periods.</li>
                <li className="flex items-center gap-2"><span className="size-2 rounded-full bg-accent" /> Work samples available within the funder's age window.</li>
              </ul>
            </Card>
          </section>

          <section>
            <h3 className="font-heading text-xl mb-3">What you'll write</h3>
                ...
                <Card key={s.key} className="p-4">
                  <div className="flex items-start justify-between gap-3 mb-1 flex-wrap">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {s.required && <Badge variant="warn">Required</Badge>}
                    </div>
                  </div>
                  <div className="text-[15px] leading-relaxed text-foreground mt-1">{s.prompt}</div>
                  {s.notes && <div className="mt-2 text-xs text-muted-foreground italic">{s.notes}</div>}
                </Card>
              ))}
            </ol>
          </section>
          ...
          <section>
            <h3 className="font-heading text-xl mb-3">Work sample spec</h3>
            <p className="text-sm text-muted-foreground leading-relaxed">{opp.work_sample_spec}</p>
          </section>

          <section>
            <h3 className="font-heading text-xl mb-3">Funder's stated values</h3>
            <ul className="space-y-2 text-sm">
              {opp.values.map((v) => (
        ...
        <aside className="col-span-12 lg:col-span-4 space-y-5">
          <Card className="p-5">
            ...
            <ul className="space-y-3 text-sm">
              <Meta icon={<Calendar size={13} />} label="Deadline">{formatDate(opp.deadline)} · <span className="text-primary">{relativeDeadline(opp.deadline)}</span></Meta>
